...
class TestTree(base.BaseTestCase):
    ...
    def test_get_consumer_nodes_collects_from_every_rp(self):
        ...
        mock_client.get.side_effect = [
            ...
            # rp1
            {'allocations': {uuids.consumer1: {'resources': {}},
                             uuids.consumer2: {'resources': {}}}},
            # rp2, consumer2 allocates from both rp1 and rp2
            {'allocations': {uuids.consumer2: {'resources': {}},
        ...
        self.assertIn(
            mock.call('/allocations/%s' % uuids.consumer1),
            consumer_calls)
        self.assertIn(
            mock.call('/allocations/%s' % uuids.consumer2),
            consumer_calls)
        self.assertIn(
            mock.call('/allocations/%s' % uuids.consumer3),
            consumer_calls)
    ...
    def test_add_consumers_to_the_graph(self):
        ...
        child1 = graph.RpNode(
            {'uuid': uuids.child1_rp, 'name': 'child1'})
        child2 = graph.RpNode(
            {'uuid': uuids.child2_rp, 'name': 'child2'})
        root = graph.RpNode(
            {'uuid': uuids.root_rp, 'name': 'root'})
        ...
        self.assertIn(
            graph.AllocationEdge(g.get_node_by_id(uuids.consumer1),
        ...
        self.assertIn(
            ...
            g.edges)
        self.assertIn(
            graph.AllocationEdge(g.get_node_by_id(uuids.consumer2),
